...
class PCFGParser(Parser):
    ...
    def get_best_parse(self, sentence):
        ...
        #Is the proper CKY table, saves the best score for each label in each cell
        chart = [[{} for _ in range(sentence_length + 1)]
                for _ in range(sentence_length)]
        #Is the parallel table that saves how we got each label
        # "NN" -> ("lex", "dog")
        # "NP" -> ("unary", "NN")
        # "S" -> ("binary", 2, "NP", "VP") here i could split in 2
        backpointers = [[{} for _ in range(sentence_length + 1)]
                for _ in range(sentence_length)]
        #Get all the tags from the lexicon
        all_tags = self.lexicon.get_all_tags()

        for begin, word in enumerate(sentence):
            cell_scores = chart[begin][begin + 1]
            cell_backpointers = backpointers[begin][begin + 1]
            for tag in all_tags:
                score = self.lexicon.score_tagging(word, tag) # How possible is to have this label
                if score > 0.0:
                    self._update_cell(cell_scores, cell_backpointers, tag,
                            score, ("lex", word)) # if possible update the cell
            self._apply_unary_rules(cell_scores, cell_backpointers) #Cierre unario

        for span in range(2, sentence_length + 1):
            for begin in range(sentence_length - span + 1):
                end = begin + span
                cell_scores = chart[begin][end]
                cell_backpointers = backpointers[begin][end]
                for split in range(begin + 1, end): #I try each possible split of the cell
                    left_cell = chart[begin][split]
                    right_cell = chart[split][end]
                    if not left_cell or not right_cell:
                        continue
                    for left_label, left_score in left_cell.items():
                        for rule in \
                                self.grammar.get_binary_rules_by_left_child(
                                        left_label): # I look for Grammar rules that start wirh that left label
                            right_score = right_cell.get(rule.right_child)
                            if right_score is None:
                                continue
                            score = rule.score * left_score * right_score
                            self._update_cell(cell_scores, cell_backpointers,
                                    rule.parent, score,
                                    ("binary", split, rule.left_child,
                                     rule.right_child))
                if cell_scores:
                    self._apply_unary_rules(cell_scores, cell_backpointers) #Cierre unario

        final_cell = chart[0][sentence_length]
        if not final_cell:
            tree = Tree("ROOT", [Tree("S")])
            return TreeBinarization.unbinarize_tree(tree)

        best_root = "ROOT"
        if best_root not in final_cell:
            best_root = max(final_cell, key=final_cell.get)

        tree = self._build_tree(sentence, backpointers, 0, sentence_length,
                best_root)
        if best_root != "ROOT":
            tree = Tree("ROOT", [tree])
        return TreeBinarization.unbinarize_tree(tree)

    def _update_cell(self, cell_scores, cell_backpointers, label, score,
            backpointer):
        old_score = cell_scores.get(label, 0.0)
        if score <= old_score:
            return False
        cell_scores[label] = score
        cell_backpointers[label] = backpointer
        return True

    def _apply_unary_rules(self, cell_scores, cell_backpointers):
        agenda = collections.deque(cell_scores.keys())
        while agenda:
            child = agenda.popleft()
            child_score = cell_scores[child]
            for rule in self.grammar.get_unary_rules_by_child(child):
                score = rule.score * child_score
                if self._update_cell(cell_scores, cell_backpointers,
                        rule.parent, score, ("unary", rule.child)):
                    agenda.append(rule.parent)

    def _build_tree(self, sentence, backpointers, begin, end, label):
        backpointer = backpointers[begin][end][label]
        pointer_type = backpointer[0]

        if pointer_type == "lex":
            return Tree(label, [Tree(backpointer[1])])

        if pointer_type == "unary":
            child_tree = self._build_tree(sentence, backpointers, begin, end,
                    backpointer[1])
            return Tree(label, [child_tree])

        split = backpointer[1]
        left_label = backpointer[2]
        right_label = backpointer[3]
        left_tree = self._build_tree(sentence, backpointers, begin, split,
                left_label)
        right_tree = self._build_tree(sentence, backpointers, split, end,
                right_label)
